...
public class AccountUtil{
    ...
    public static Account randomAccount() {
        try {
            ...
            ECKeyGenerationParameters keygenParams = new ECKeyGenerationParameters(CURVE, SECURE_RANDOM);//利用椭圆曲线和随机种子创建一个<公私钥对>
            generator.init(keygenParams);
            AsymmetricCipherKeyPair keypair = generator.generateKeyPair();//获取到keypair
            ECPrivateKeyParameters ecPrivateKeyParameters = (ECPrivateKeyParameters) keypair.getPrivate();//获取到私钥
            ECPublicKeyParameters pubParams = (ECPublicKeyParameters) keypair.getPublic();//获取到公钥

            BigInteger bigIntegerPrivateKey = ecPrivateKeyParameters.getD();//私钥
            byte[] bytesPublicKey = pubParams.getQ().getEncoded(COMPRESSED);//公钥byte【】数组
