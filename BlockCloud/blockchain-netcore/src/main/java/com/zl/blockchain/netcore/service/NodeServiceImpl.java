...
public class NodeServiceImpl implements NodeService {
    ...
    public Node queryESNode(String ip) {
        Node Node = nodeDao.queryESNode(ip);
        if(Node == null){
            return null;
        }
        return Node;
    ...
    public List<Node> queryAllESNodes() {
        List<Node> Nodes = nodeDao.queryAllESNodes(); //调用nodedap的queryAllNodes（）
        return Nodes; //返回node列表
    }

    @Override
    public void addESNode(Node node) {
        if(nodeDao.queryESNode(node.getIp()) != null){ //如果查询到节点已存在，则不添加
            return;
        }
        nodeDao.addESNode(node); //nodedap添加Node
    }

    @Override
    public void updateESNode(Node node) {
        Node Node = nodeDao.queryESNode(node.getIp());
        if(Node == null){ //如果查询到节点为空，则不更新
            return;
        }
        nodeDao.updateESNode(node);
    }

    @Override
    public void deleteESNode(String ip) {
        nodeDao.deleteESNode(ip);
    }
