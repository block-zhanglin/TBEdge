...
public class HttpServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, FullHttpRequest fullHttpRequest) {
        ...
        if("/".equals(requestApi)){ //如果请求api为“/”
            ...
            PostBlockResponse response = httpServerHandlerResolver.postBlock(request);//构建response
            responseMessage = JsonUtil.toString(response);
            ...
            responseMessage = JsonUtil.toString(response);
        }else if(API.EsFull.equals(requestApi)){
            Esfullrequest esfullrequest=JsonUtil.toObject(requestBody,Esfullrequest.class);
            Esfullresponse esfullresponse=httpServerHandlerResolver.esfull(requestIp,esfullrequest);
            responseMessage=JsonUtil.toString(esfullresponse);
        }
        else {
