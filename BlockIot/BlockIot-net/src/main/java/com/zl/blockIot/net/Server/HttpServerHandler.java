    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, FullHttpRequest fullHttpRequest) {
        try{
            if(fullHttpRequest instanceof  FullHttpRequest){
                ...
                }else if(requestApi.equals(API.EstoIotrequest)){
                    EstoIotrequest estoIotrequest=JsonUtil.toObject(requestBody,EstoIotrequest.class);
                    System.out.println(estoIotrequest.getTaskNumber()+estoIotrequest.getEsNumber());
                    EstoIotResponse estoIotResponse=httpServerHandlerResolver.estoIot(estoIotrequest);
                    responseMessage=JsonUtil.toString(estoIotResponse);
                }else if(requestApi.equals(API.EstoIotdatarequest)){
                    EstoIotdatarequest estoIotdatarequest=JsonUtil.toObject(requestBody,EstoIotdatarequest.class);
                    System.out.println(estoIotdatarequest.getDatalocation());
                    EstoIotdataResponse estoIotdataResponse=httpServerHandlerResolver.estoIotdata(estoIotdatarequest);
                    responseMessage=JsonUtil.toString(estoIotdataResponse);
                }
                else {
