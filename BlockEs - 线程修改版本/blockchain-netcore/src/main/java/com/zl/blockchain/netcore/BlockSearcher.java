...
public class BlockSearcher {
    ...
    private void searchNodesBlocks() {
        ...
        List<com.zl.blockchain.dto.Node> nodeList=committee.getNodes();//所有的委员会节点
        List<Node>nodes=new ArrayList<>();//向委员会中的节点搜索（从dto层转换为网络层）

        for (com.zl.blockchain.dto.Node node:nodeList){
            Node node1=new Node();
            node1.setBlockchainHeight(node.getBlockchainHeight());
            node1.setIp(node.getIp());
            nodes.add(node1);
        }
        ...
        //搜索所有委员会节点最后一个区块，如果获取到f+1个相同的最后区块，则尝试和其同步【为了防止作恶节点不进行共识，直接加区块。将自己变成最长链】
//
//        long limit=Integer.valueOf(BlockSetting.CommitteeCount)*1/3+1;
//        HashMap<BlockDto,Integer> hm=new HashMap<>();
//         Node nt=new Node();
//        for(Node node:nodes){
...
//            getBlockRequest.setBlockHeight(getBlockchainHeightResponse.getBlockchainHeight());
//            GetBlockResponse getBlockResponse=nodeClient.getBlock(getBlockRequest);
//            BlockDto bd=getBlockResponse.getBlock();
//            if(hm.containsKey(bd)){
//                Integer n=hm.get(bd);
//                if(n+1>=limit){
//                    nt=node;
//                    break;
...
//            }
//        }
//        bs(blockchainCore,slaveBlockchainCore,nt);
        ...
        for(Node node:nodes){
              ...
              new Thread(()->bs(blockchainCore,slaveBlockchainCore,node)).start();
...
//            }
    ...
    private  void bs(BlockchainCore blockchainCore,BlockchainCore slaveBlockchainCore,Node node){
        try{
            if(!node.getIp().equals(API.Esip)){
                searchNodeBlocks(blockchainCore,slaveBlockchainCore,node);
            }else {
    ...
    public void searchNodeBlocks(BlockchainCore masterBlockchainCore, BlockchainCore slaveBlockchainCore, Node node) {
        ...
        long nodeBlockchainHeight=nodeService.queryNode(node.getIp()).getBlockchainHeight();
        //本地区块链高度大于等于远程节点区块链高度，此时远程节点没有可以同步到本地区块链的区块。
        if(masterBlockchainHeight >= nodeBlockchainHeight){
            //System.out.println(masterBlockchainHeight+"----"+nodeBlockchainHeight);
            //System.out.println("本地区块高度>="+node.getIp());
            return;
        }
       // System.out.println("本地区块链高度"+masterBlockchainHeight+"----"+"节点区块链高度"+nodeBlockchainHeight);
        //本地区块链与node区块链是否分叉？
    ...
    private void duplicateBlockchainCore(BlockchainCore fromBlockchainCore,BlockchainCore toBlockchainCore) {
        ...
        while (true){
            ...
            toBlockchainCore.getBlockchainDatabase().MineraddBlockDto(Model2DtoTool.block2BlockDto(nextBlock));
//            toBlockchainCore.addBlock(nextBlock);
    ...
    private void synchronizeBlocks(BlockchainCore blockchainCore, Node node, long startBlockHeight) {
        while (true){
            ...
            //System.out.println("开始搜索区块");
            boolean isAddBlockSuccess = blockchainCore.getBlockchainDatabase().MineraddBlockDto(remoteBlock);
//            boolean isAddBlockSuccess = blockchainCore.addBlockDto(remoteBlock);
            ...
            if(startBlockHeight>node.getBlockchainHeight()){  //不能超过node节点有的区块高度
                break;
            }
